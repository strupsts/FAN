...
class ReceiptRow(Base):
    ...
    total_currency: Mapped[str] = mapped_column(String(3), default="CAD")
    image_ref: Mapped[str | None] = mapped_column(String(1000), nullable=True)
...
class ReceiptItemRow(Base):
    ...
    total_price_amount: Mapped[Decimal] = mapped_column(Numeric(12, 2))
    total_price_currency: Mapped[str] = mapped_column(String(3), default="CAD")

    category: Mapped[str] = mapped_column(String(100), default="unknown")
    bucket: Mapped[str] = mapped_column(String(100), default="unknown")

    quantity: Mapped[float | None] = mapped_column(nullable=True)
    unit_price_amount: Mapped[Decimal | None] = mapped_column(Numeric(12, 2), nullable=True)
    unit_price_currency: Mapped[str | None] = mapped_column(String(3), nullable=True)
